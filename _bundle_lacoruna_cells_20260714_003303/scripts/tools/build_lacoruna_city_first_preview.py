...
def get_province() -> tuple[dict, dict, Polygon]:
	...
	return province, geom, province_poly
...
def build_cells(province_poly: Polygon, city_cell: Polygon) -> list[tuple[str, str, Polygon]]:
	...
	remaining = largest_polygon(remaining)
	...
	west = choose_piece(west_split, lambda p: -p.representative_point().x)
	interior = largest_polygon(remaining.difference(west))
	if interior is None:
	...
	south = choose_piece(interior_split, lambda p: p.representative_point().y)
	east = largest_polygon(interior.difference(south))
	if east is None:
