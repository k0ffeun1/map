...
#
# 2026-07-12: "La CoruÃ±a" — маяк Torre de Hércules на самом кончике
...
# клине земли, который patch_sevilla_coastal_access.py отобрал у "CÃ¡diz"/
...
# ради визуала карты — не путать с "La CoruÃ±a" выше, там наоборот важно
# было настоящее место. Точная позиция подобрана пользователем ВРУЧНУЮ —
# перетащена мышкой в игре (см. ProvinceCityMarkersLayer.gd/кнопка
# "Сохранить города" в TileMapViewer.gd) и перенесена сюда из
# assets/province_cities_iberia.json, чтобы не потеряться при следующей
# перегенерации этим скриптом.
CITY_POSITION_OVERRIDES = {
	"La CoruÃ±a": (-8.4083, 43.3712),
	"Sevilla": (-6.1765, 36.9264),
}
