...
Механика — максимально простая, никакой органики:
1. Контур провинции (assets/provinces.json) используется ТОЛЬКО чтобы
   обрезать сетку — саму границу не меняем и не анализируем (нет ни волнения,
   ни выделения "какой кусок ребра лежит на границе провинции", в отличие
   от build_cells_test.py/_split_open_border_chains).
2. bbox провинции делится ПРЯМЫМИ линиями — обычная равномерная сетка
   GRID_ROWS x GRID_COLS (2x2 по умолчанию = 4 клетки).
3. Каждый прямоугольник сетки обрезается пересечением с полигоном провинции
   (shapely intersection) — чисто геометрическая операция.
4. Внутренние рёбра клеток — прямые линии сетки, без волнения.
...
def main() -> None:
	...
	for row in range(GRID_ROWS):
		for col in range(GRID_COLS):
			...
			clipped = rect.intersection(province_poly)
			if clipped.is_empty:
				continue
