...
просто "это вода/не вода", один слой для проверки перед тем как резать
generate_sea_cells.py на клетки). Отдельный файл/скрипт от
generate_sea_cells.py — это разные задачи (маска целиком vs нарезка на
клетки), не путать.
...
ВАЖНО (та же находка, что и в generate_sea_cells.py/load_land_pieces):
