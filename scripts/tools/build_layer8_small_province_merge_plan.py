...
proposes logical merges for genuinely small families.

User-locked policy:
- current Layer-8 Slovenia stays unchanged;
- current Greater London aggregate stays unchanged;
- declared historical/strategic islands are never absorbed;
- all other logical provinces below 500 km² must receive a deterministic merge
  target, even when that target is across a modern country boundary;
- current country is a strong preference, not a hard boundary, because the map
  is for a historical strategy and the user explicitly requested consolidation;
- the future Low Countries superregion boundary must remain possible.
...
def main() -> None:
    ...
    if any(v != EXPECTED_LAYER8_COUNT for v in coverage.values()):
        raise RuntimeError(f"Layer-8 coverage mismatch: {coverage}")
    ...
    archipelago_region_names = {
        str(name)
        for zone in archipelago_zones
        for name in zone.get("region_names", [])
    }
    low_countries_core = load_low_countries_core()

    # Layer 8 exploded many source MultiPolygons into foo, foo_2, foo_3...
    # Reconstruct those source-feature families before deciding what is truly small.
    ...
    for index, key in enumerate(sorted(family_members.keys())):
        ...
        merged_geom = unary_union([geometries[p] for p in members])
        ...
        if prefix == "slovenia":
            ...
            legacy_text = " ".join(norm_text(identities[p].get("legacy_id")) for p in members)
            names = {norm_text(identities[p].get("name")) for p in members}
            if "greater_london" in legacy_text or "greater london" in names or "большой лондон" in names:
    ...
    def crosses_low_countries_contract(source: dict[str, Any], target: dict[str, Any]) -> bool:
        source_inside = source["region_name"] in low_countries_core
        target_inside = target["region_name"] in low_countries_core
    ...
    def target_allowed(source: dict[str, Any], target: dict[str, Any]) -> bool:
        ...
        if target["hard_lock_reason"]:
            return False
        if crosses_low_countries_contract(source, target):
            return False
        # Prevent merge cycles: an unprotected small target must be strictly larger.
        if target["logical_small"] and not target["protected_group_id"]:
            if float(target["area_km2"]) <= float(source["area_km2"]) + EPS:
                return False
        return True
    ...
    def score_touching(source: dict[str, Any], target: dict[str, Any]) -> tuple[float, dict[str, float]]:
        ...
        same_country = 1.0 if source["country_prefix"] == target["country_prefix"] else 0.0
        combined = unary_union([sg, tg])
        compact_delta = safe_compactness(combined) - safe_compactness(tg)
    ...
    def nearest_fallback(source: dict[str, Any]) -> tuple[dict[str, Any], float, str] | None:
        """Resolve isolated small families without making modern borders a hard wall.

        Priority:
        1. same region + same country;
        2. same region;
        3. same country;
        4. global nearest allowed family.

        For Caribbean declared archipelago regions, same-region candidates are
        explicitly allowed across sea. For other isolated territories this is a
        last-resort logical province consolidation, still preserving source lineage.
        """
        buckets: list[tuple[str, list[dict[str, Any]]]] = []
        same_region_same_country = [
            f for f in families
            if target_allowed(source, f)
            and f["region_name"] == source["region_name"]
            and f["country_prefix"] == source["country_prefix"]
        ]
        same_region = [
            f for f in families
            if target_allowed(source, f)
        ...
        global_allowed = [f for f in families if target_allowed(source, f)]
        buckets.extend([
            ("same_region_same_country", same_region_same_country),
            ("same_region", same_region),
            ("same_country", same_country),
            ("global_nearest", global_allowed),
        ])
        for label, bucket in buckets:
            ...
            ranked = sorted(
                (
                    float(source["geometry"].distance(target["geometry"])),
                    0 if not target["logical_small"] or target["protected_group_id"] else 1,
                    -float(target["area_km2"]),
                    target["family_key"],
                    target,
                )
    ...
    for family in families:
        ...
        if family["hard_lock_reason"]:
            action = {**base, "status": "LOCKED_APPROVED", "reason": family["hard_lock_reason"]}
            ...
            action = {**base, "status": "KEEP", "reason": "logical_family_area_at_least_500_km2"}
        else:
            ...
            if candidates:
                ...
                action = {
                    **base,
                    ...
                    "reason": "best_touching_neighbor_with_historical_preferences",
                    "target_family_id": target["family_id"],
                    "target_family_key": target["family_key"],
                    "target_province_id": target["anchor_province_id"],
                    "target_name": target["name"],
                    "target_country_prefix": target["country_prefix"],
                    "target_region_name": target["region_name"],
                    "cross_country": target["country_prefix"] != family["country_prefix"],
                    "score": round(score, 6),
                    "score_parts": parts,
                }
            else:
                fallback = nearest_fallback(family)
                if fallback is None:
                    action = {**base, "status": "REVIEW", "reason": "no_allowed_target_found"}
                else:
                    target, distance, selection_tier = fallback
                    is_archipelago = family["region_name"] in archipelago_region_names
                    action = {
                        **base,
                        "status": "AUTO_MERGE_ARCHIPELAGO" if is_archipelago else "AUTO_MERGE_NEAREST",
                        "reason": "isolated_small_family_nearest_historically_preferred_target",
        ...
        if len(family["member_ids"]) > 1:
            for pid in family["member_ids"]:
                if pid == family["anchor_province_id"]:
                    continue
                if family["hard_lock_reason"]:
                    piece_status = "LOCKED_APPROVED"
                    piece_target = ""
                    reason = family["hard_lock_reason"]
                else:
                    piece_status = "TECHNICAL_FAMILY_REASSEMBLY"
                    piece_target = family["anchor_province_id"]
                    reason = "same_source_feature_render_piece"
                piece_actions.append({
                    ...
                    "status": piece_status,
                    "target_province_id": piece_target,
                    "family_id": family["family_id"],
                    "reason": reason,
                })
    ...
    auto_actions = [a for a in family_actions if a["status"].startswith("AUTO_MERGE")]
    review_actions = [a for a in family_actions if a["status"] == "REVIEW"]
    cross_country_auto_merges = sum(1 for a in auto_actions if a.get("cross_country"))
    hard_lock_violations = 0
    protected_absorption_violations = 0
    low_countries_bad_targets = 0
    non_increasing_small_target_count = 0
    for action in auto_actions:
        source = family_by_id[action["family_id"]]
        target = family_by_id.get(str(action.get("target_family_id", "")))
        if source["hard_lock_reason"] or (target and target["hard_lock_reason"]):
            hard_lock_violations += 1
        if source["protected_group_id"]:
            protected_absorption_violations += 1
        if target and crosses_low_countries_contract(source, target):
            low_countries_bad_targets += 1
        if target and target["logical_small"] and not target["protected_group_id"]:
            if float(target["area_km2"]) <= float(source["area_km2"]) + EPS:
                non_increasing_small_target_count += 1

    unprotected_small_actions = [
        a for a in family_actions
        if family_by_id[a["family_id"]]["logical_small"]
        and not family_by_id[a["family_id"]]["protected_group_id"]
        and not family_by_id[a["family_id"]]["hard_lock_reason"]
    ]
    unresolved_unprotected_small = [a for a in unprotected_small_actions if not a["status"].startswith("AUTO_MERGE")]
    ...
    validations = {
        ...
        "non_increasing_small_target_count": non_increasing_small_target_count,
        "unresolved_unprotected_small_count": len(unresolved_unprotected_small),
    ...
    summary = {
        ...
        "automatic_merge_count": len(auto_actions),
        "cross_country_automatic_merge_count": cross_country_auto_merges,
    ...
    doc = {
        "schema_version": 2,
        "format": "layer8_small_province_merge_plan/v2",
        ...
        "policy": {
            ...
            "modern_country_boundary_is_preference_not_hard_constraint": True,
            "protected_historical_island_absorption_forbidden": True,
            "slovenia_layer8_locked": True,
            "greater_london_layer8_locked": True,
            "low_countries_future_superregion_boundary_locked": True,
            "technical_split_source_pieces_reassembled_before_size_decision": True,
        },
        "summary": summary,
    ...
    lines = [
        ...
        f"- Автоматических merge маленьких logical family: **{summary['automatic_merge_count']}**",
        f"- Из них пересекают современную границу страны: **{summary['cross_country_automatic_merge_count']}**",
        f"- На REVIEW: **{summary['review_count']}**",
        ...
        f"- Циклически опасные small→small target: **{non_increasing_small_target_count}**",
        f"- Неразрешённых НЕзащищённых маленьких logical family: **{len(unresolved_unprotected_small)}**",
        f"- Словения полностью locked: **{validations['slovenia_all_locked']}**",
        f"- Большой Лондон locked ровно одной family: **{validations['greater_london_locked']}**",
        "",
    ...
    lines.extend(["", "## Все автоматические merge маленьких logical family", ""])
    for action in auto_actions:
        border = "cross-country" if action.get("cross_country") else "same-country"
        lines.append(
            f"- **{action['name']}** ({action['area_km2']:.1f} км²) → "
            f"**{action.get('target_name', '?')}** [{border}] — `{action['status']}`"
        )
    ...
    if non_increasing_small_target_count:
        failures.append(f"cycle-risk small targets: {non_increasing_small_target_count}")
    if unresolved_unprotected_small:
        failures.append(f"unresolved unprotected small families: {len(unresolved_unprotected_small)}")
    ...
    if not validations["greater_london_locked"]:
        failures.append(f"Greater London lock expected exactly 1 family, got {len(london_actions)}")
