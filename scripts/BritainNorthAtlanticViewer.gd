...
const SELECT_BORDER := Color(1.0, 0.92, 0.46, 1.0)
...
func _input(event: InputEvent) -> void:
	...
	if mouse == null or not mouse.pressed or mouse.button_index != MOUSE_BUTTON_LEFT:
		return
	var hit := _hit(get_global_mouse_position())
	if not hit.is_empty():
...
func _load_data() -> void:
	...
	if str(cdoc.get("format", "")) != "britain_north_atlantic_subdivisions/v1":
		...
		return
	if int(pdoc.get("gameplay_province_count", 0)) != EXPECTED_PROVINCES or int(cdoc.get("cell_count", 0)) != EXPECTED_CELLS:
		_fail("неожиданные totals")
		return
	_provinces.clear()
	_cells.clear()
	_world_bbox.clear()
...
func _show_selection(item: Dictionary) -> void:
	...
	if item.has("parent_id"):
		_selection_label.text = "КЛЕТКА\n%s\nПровинция: %s\nТерритория: %s\nПлощадь: %.1f км²" % [str(item.get("id", "")), str(item.get("parent_name", "")), str(item.get("territory", "")), float(item.get("area_km2", 0.0))]
	else:
		_selection_label.text = "ИГРОВАЯ ПРОВИНЦИЯ\n%s\nID: %s\nТерритория: %s\nКлеток: %d\nПлощадь: %.1f км²" % [str(item.get("name", "")), str(item.get("id", "")), str(item.get("territory", "")), int(item.get("target_cell_count", 0)), float(item.get("area_km2", 0.0))]

func _focus_all() -> void:
	if not is_instance_valid(_camera) or _world_bbox.size() < 4:
		return
	var minx := float(_world_bbox[0])
	var miny := float(_world_bbox[1])
