extends Node2D
## Interactive manual editor for the world-region layer.
##
## I  - world regions (handled by WorldRegionsDraftViewer)
## E  - toggle editor
## LMB - choose source REGION
## Alt+LMB - choose one layer-8 province piece
## RMB - choose target region
## Enter - append persistent override
## Z - undo last override
## S - save overrides
## Esc - clear selection / leave edit mode
...
const PROVINCES_PATH := "res://assets/map_geometry/provinces.json"
...
const TARGET_COLOR := Color(0.25, 1.0, 0.48, 1.0)
...
var _source_mode := "" # region / province
...
func _select_source_province(point: Vector2) -> void:
	...
	if hit.is_empty():
		_show("Редактор: под курсором нет province-piece")
		return
	_clear_source_highlight()
	...
	queue_redraw()
	_update_panel()
	_show("ИСТОЧНИК-кусок: %s (%s) • ПКМ по целевому региону" % [_source_province_name, _source_region_name])
...
func _apply_pending_override() -> void:
	...
	if _save_overrides():
		_show("Override сохранён: %s → %s. После rebuild граница исчезнет." % [_source_province_name if _source_mode == "province" else _source_region_name, _target_region_name])
	else:
...
func _ensure_province_geometry() -> void:
	...
	if not FileAccess.file_exists(PROVINCES_PATH):
		return
	var parsed: Variant = JSON.parse_string(FileAccess.get_file_as_string(PROVINCES_PATH))
	if typeof(parsed) != TYPE_DICTIONARY:
		return
	for raw in (parsed as Dictionary).get("provinces", []):
		...
		if rings.is_empty():
			continue
		var bbox := _bbox_for_rings(rings)
		var pid := str(entry.get("id", ""))
		var assignment: Dictionary = _assignments_by_id.get(pid, {})
		_province_records.append({
			"province_id": pid,
			"name": str(assignment.get("name", pid)),
			"bbox": bbox,
			"rings": rings,
		})
...
func _province_at_point(point: Vector2) -> Dictionary:
	for i in range(_province_records.size() - 1, -1, -1):
		var item: Dictionary = _province_records[i]
...
func _draw() -> void:
	if not _edit_mode or _source_mode != "province" or _source_province_rings.is_empty():
		return
	for ring in _source_province_rings:
...
func _update_panel() -> void:
	if not is_instance_valid(_panel):
		return
	_panel.visible = _edit_mode
	if not _edit_mode:
		return
	var source := "—"
	if _source_mode == "region":
		source = "РЕГИОН: %s" % _source_region_name
	elif _source_mode == "province":
		source = "КУСОК: %s (%s)" % [_source_province_name, _source_region_name]
	var target := _target_region_name if not _target_region_name.is_empty() else "—"
	_label.text = "РЕДАКТОР РЕГИОНОВ\nИсточник: %s\nЦель: %s\n\nЛКМ — регион\nAlt+ЛКМ — один province-piece\nПКМ — целевой регион\nEnter — применить\nZ — отменить последний override\nS — сохранить\nEsc — очистить/выйти\n\nOverrides: %d\nФайл: %s" % [source, target, _operations.size(), _last_saved_path if not _last_saved_path.is_empty() else OVERRIDES_PATH]
